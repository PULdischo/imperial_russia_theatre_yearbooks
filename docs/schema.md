...
| `rank_or_title` | string, nullable | Despite the one name, corpus-wide content shows this holds two genuinely different kinds of fact, and their mix varies sharply by `entity_type`. **RANK** — institutionally conferred status: a Table-of-Ranks civil/court grade (`ст. сов.`, `колл. сов.`, `надв. сов.`, `тит. сов.`, `неимѣющій чина`...) or a court honorific (`Солистъ Двора Его Императорскаго Величества`, `Заслуженный артистъ Императорскихъ театровъ`) — dominant in Administrators (1483 non-blank) and much of TheaterSchoolStaff (543). **TITLE** — how the Yearbook labels the entry: an occupational role (`Капельмейстеръ`, `Концертмейстеръ`, `Репетиторъ балета`, a dual-role note like `(онъ же и библіотекарь Музыкальной библіотеки)`), a subject taught functioning as a teacher's title for TheaterSchoolStaff (`Танцы`, `Музыка`, `Французскій языкъ`, `Исторія`), or a workshop specialty functioning as a costumer/wigmaker's title for ProductionTeam (`Мужскіе парики`, `Женскіе костюмы балетной и французской труппъ`) — dominant in Musicians (52) and BalletArtists (60). Two further shapes land in this field but are **neither** rank nor title: BalletArtists' `1-я`/`2-я`/`3-я`/`4-я` values, confirmed by query (not assumed) to be a name-disambiguation device rather than a seniority marker — every instance checked pairs a distinct patronymic with each ordinal within a shared surname on the same page (e.g. Иванова: Елена Павловна=1-я, Марія Ѳедоровна=2-я, Александра Ивановна=3-я, Наталья Евдокимовна=4-я) — the same convention already printed directly into `family_name` elsewhere in the corpus (see `family_name`'s own note above, `Алексѣева 1-я`); a stray instance of that convention, not a new kind of fact — relocated onto `family_name_clean` rather than tagged-and-dropped. And dual-role/cross-reference notes (`(онъ же и режиссеръ)`, `(см. СПБ. балетъ)`) — administrative notes that happen to land here, conceptually closer to `tenure_note_text` — relocated onto `tenure_note_text_clean` (scan-verified first: print order is note-then-date, e.g. `Вальцъ, Карлъ Ѳедоровичъ (онъ же и декораторъ) (съ 3 октября 1861 г.)`), the same contamination pattern already fixed for `instrument` (known_issues.md #66). **This split is implemented, not just characterized** — `analysis.person_entry.rank_clean`/`title_clean` (pure SQL derivation, `raw.person_entry.rank_or_title` itself stays untouched/verbatim), `rank_or_title_excluded_reason` (audit-only, tags the 48 rows that are neither rank nor title after the two relocations above), and `research.person_appearance.rank`/`title` in place of the old `rank_or_title` column. Full write-up, including the classification's validation (0 mismatches against a Python reference across all 2208 non-blank values) and a DuckDB RE2-vs-Cyrillic gotcha hit along the way, in known_issues.md #67. |
...
| `instrument` | string, nullable | Musicians only. A structured extraction sitting alongside `tenure_note_text`, not derived from it downstream — the opposite design call from `heading_path`'s department/position split above, and deliberately so. `heading_path` stayed one verbatim field because the department/position boundary is a visual/typographic judgment call (indentation, bold weight) a model can't reliably reproduce from text alone. An instrument name isn't that: it's a closed vocabulary (~40 known instrument names, see `_KNOWN_INSTRUMENTS` in `pipeline/parse_and_validate.py`) appearing as a literal, recognizable token in the sentence — the same kind of well-defined pattern that already justifies asking the model for structured `service_periods` (→ `person_entry_service`) alongside the verbatim tenure note, not a reason to avoid a dedicated column. `tenure_note_text` and `instrument` are also not simply the same fact in two places: cross-reference-stub rows routinely carry an instrument with no date in the same row (the date lives on a different "home" page), so `instrument` is independently useful information the verbatim field doesn't always carry in a fixed position. In practice `tenure_note_text` is genuinely messy free text (missing parens, missing dates, cross-reference-prefixed, dual-role parentheticals, compound dates, service/death notes appended) — every edge shape found and fixed in the #54-#65 issue arc was tractable specifically because the extraction lives in Python (a real lexicon, a boundary-safety check, page/index-keyed hand-fixes checked against scans), not as SQL regex reconstructing it from the raw sentence. |
